...
# Import the universal loader (optional). Some environments may not have the
# full `loaders` module or its native dependencies (eg. python-magic). Make
...
try:
    ...
    def download_and_load(path_or_url: str):
        """Lightweight fallback to download or read a document and return
        (text_content, detected_format).

        This is intentionally simple: it will try to read local text files,
        fetch URLs, and use PyMuPDF for basic PDF extraction when available.
        It avoids importing heavyweight or system-dependent libraries at
        module import time.
        """
        try:
            # Local file path
            if os.path.exists(path_or_url):
                ...
                if ext == '.pdf':
                    ...
                    with open(path_or_url, 'r', encoding='utf-8', errors='ignore') as f:
                        return f.read(), ext or 'text'

            # Try fetching from URL
            resp = requests.get(path_or_url, timeout=20)
            resp.raise_for_status()
            content_type = resp.headers.get('Content-Type', '')
            if 'pdf' in content_type or path_or_url.lower().endswith('.pdf'):
                # Save to temp file and attempt PDF extraction
            ...
            raise ImportError(f"download_and_load fallback failed: {e}")
